 ...
 */
export function createDOMEmergencyMarkers(mapContainer: HTMLElement) {
 ...
 */
export function createMapboxEmergencyMarkers() {
  ...
  if (window.mapboxgl && window.mapInstance) {
    ...
    emergencyCoordinates.forEach((coords, index) => {
      ...
      try {
        // Ensure coords is a tuple with exactly two elements by explicitly specifying the type and indexes
