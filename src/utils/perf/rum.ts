  ...
  const flush = async () => {
    try {
      ...
      const payload: any = {
        ...
        lcp_ms: state.lcp != null ? Math.round(state.lcp) : null,
        inp_ms: state.inp != null ? Math.round(state.inp) : null,
        cls: state.cls != null ? Number(state.cls.toFixed?.(3) ?? state.cls) : null,
        fcp_ms: state.fcp != null ? Math.round(state.fcp) : null,
        ttfb_ms: state.ttfb != null ? Math.round(state.ttfb) : null,
